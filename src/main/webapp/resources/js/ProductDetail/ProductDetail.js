	...
	$.ajax({
			...
			var perBtn = "";
				...
				for(var i = e.p.beginPage ; i <= e.p.endPage; i++ ){
					liTag += '<li style="margin-right:10px"><button class="list-paginator__page sm selected" type="button">'+i+'</button>';
...
$("#product-select").on("change", function(){
	...
	$('#_optionSelectList').append('<li style="padding: 20px;">\
									...
									</div><div><div class="btns"><button >감소</button><input class="ctn"  type="number"  value="1" dir="rtl"><button>증가</button> \
									<strong><em class="total-price">'+price.innerHTML+'</em>원</strong><button>취소</button></div></div></li>');
	...
	$(document).on("click",".btns button",function(e){
		console.log(e.target.innerText);
		var ctn = $('.ctn').val();
		if(e.target.innerText == '감소'){
